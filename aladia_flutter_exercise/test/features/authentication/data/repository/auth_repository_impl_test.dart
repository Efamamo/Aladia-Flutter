...
import 'package:aladia_flutter_exercise/core/error/exception.dart';
import 'package:aladia_flutter_exercise/core/error/failure.dart';
import 'package:aladia_flutter_exercise/features/authentication/data/model/login_response_model.dart';
  ...
  group('login ', () {
    group('when the device is online', () {
      ...
      test(
        ...
        //arrange
        when(mockAuthRemoteDataSource.login(testLoginModel))
            .thenThrow(NotFoundEmailFailure());
        //act
        final result = await repository.login(testLoginEntity);

        //assert
        expect(result, equals(Left(NotFoundEmailFailure())));
      });
    });
